...
def generate_velocity(galaxy,z_dim=True):
    ratio = galaxy[:,1]/galaxy[:,0]
    v = np.vstack((-ratio,np.ones(galaxy.shape[0]))).T
    v = v * 3/np.linalg.norm(v,axis=1).reshape(-1,1) # Normalize

    v = np.nan_to_num(v)

    if z_dim : 
        v = np.concatenate((v,np.zeros(galaxy.shape[0]).reshape(-1,1)),axis=1)
